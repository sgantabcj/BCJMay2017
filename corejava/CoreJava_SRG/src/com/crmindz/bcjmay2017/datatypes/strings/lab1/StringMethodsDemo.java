...
//		checking all the occurrences of a character in string
		...
		System.out.println("The occurrence of substring \"A\" in "+s+" is at indices: "+occuranceIndicesArray);
...
//	returning a string of indices for all the occurrences of a string or char in another string
	static String getAllOccurances(String s, String substring) {
		String occurrenceIndicesStr = "";
		int currentIndex = -1;
		while(currentIndex<s.lastIndexOf(substring)){
			currentIndex = s.indexOf(substring,currentIndex+1);
			occurrenceIndicesStr+=currentIndex+" ";
		}
		return occurrenceIndicesStr;
	}
}
